...
describe("User Creation Saga", function () {
  const generatorWaitingForRequest = cloneableGenerator(userCreationSaga)();
  ...
  const notTheSamePassword = "password2";
  const validEmail = "asdf@asdf.com";
  const invalidEmail = "fasd98uiuh";
  ...
  it("yields a put effect with a user creation failure action with a reason if there was in error in response from the server", function () {
    const clone = generatorWaitingForRequest.clone();
    clone
